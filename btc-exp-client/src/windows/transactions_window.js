...
class TransactionsWindow extends React.Component {
	...
	async fetch(){
		...
		try {
			...
			const response = await axios.get('http://localhost:8080/transactions/new', {
				params: {
	...
	async searchRaw(){
		try {
			...
			const response = await axios.get('http://localhost:8080/transactions/raw',{
				params: {
	...
	async searchDecoded(){
		try {
			...
			const response = await axios.get('http://localhost:8080/transactions/decode', {
				params: {
			...
			const second_response = await axios.get('http://localhost:8080/transactions', {
				params: {
			...
			this.setState({focusTx: {...response.data, ...second_response.data}})
	...
	render(){
		...
		return (
			<Box pad="medium" width="100%" animation="fadeIn">
				<Box round="medium" elevation="medium" background="white" pad="xsmall" height="fit-content" direction="row">
					<TextInput
						placeholder="Block Height"
	      				value={this.state.blockHeight}
	     				onChange={e => this.setState({blockHeight: e.target.value})}
	    			/>
	    			<Box round="medium" 
				...
				<Box direction="row" width="100%" gap="medium">
					<Box background="white" round="medium" elevation="medium" margin={{top:"medium"}} pad="small" overflow="scroll" style={{maxHeight: "79vh"}}>
						<DataTable
						  columns={[
						    {
						      property: 'index',
						      header: <Text className="heavy" color="#6e72e6">Index</Text>,
						      primary: true,
						      render : datum => <Text color="#68707f">{datum.index}</Text>
						    },
						    {
						      property: 'hash',
						    ...
						    {
						      property: 'time',
						      header: <Text className="heavy" color="#6e72e6">Time</Text>,
						      render: datum => (
						        <Box pad={{ vertical: 'xsmall' }} className="info-row">
						    ...
						    {
						      property: 'fee',
						      header: <Text className="heavy" color="#6e72e6">Fee</Text>,
						      render: datum => (
						        <Box pad={{ vertical: 'xsmall' }} className="info-row">
						          <Text textAlign="end">{datum.fee}</Text>
						        </Box>
						      ),
						    },
						    {
						...
						<Box round="medium" elevation="medium" background="white" pad="small" height="fit-content">
							<TextInput
								placeholder="Tx Hash"
	     					    ...
	     					    onChange={e => this.setState({queryRaw: e.target.value})}
	    					/>
	    					<Box round="medium" 
	    						 className="hor-gradient" 
	    						 pad={{vertical:"small"}} 
						...
						<Box round="medium" elevation="medium" background="white" pad="small" height="fit-content">
							<TextInput
								placeholder="Raw Tx"
	     					    ...
	     					    onChange={e => this.setState({queryTx: e.target.value})}
	    					/>
	    					<Box round="medium" 
	    						 className="hor-gradient" 
	    						 pad={{vertical:"small"}} 
				...
				<TransactionInfo block={this.state.focusTx} ref={this.infoWindow}/>
